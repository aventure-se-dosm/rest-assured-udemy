...
=======
>>>>>>> a31d3f9c6ea6127cfff58bb42b948acb6c319f37
...
/*	//keep it up for your end-of-course rethinks!
*	private String getPartialQuery(String city,  String contry, String appId) { 
*		return String.format("q=%s,%s&appid=%s", city, appId );
*	}
*/
...
=======
		...
		/**
		 * REFACTOR POINT: Use a Json with all of underlying
		 * attribute-values, so we simply rely on serialization
		 * of models, avoiding declaration or too much variable 
		 * creation and attribuitons.
		 */
...
>>>>>>> a31d3f9c6ea6127cfff58bb42b948acb6c319f37
	...
	public void deveAcessarCidadeComQueryParam() {
		...
		.queryParam("unit", "meter")
		.queryParam("appid", appIdKey)	
		.when()
			.get(getOpenWeatherMap())
		.then().log().all()
			.assertThat()
		.statusCode(HttpStatus.SC_OK)
...
=======
		...
		//.body("coord.lat", is(47.8D))		// Assertion error (wrong type): not a String to be expected, a float value rather
		//.body("coord.lon", is(9.6333)) 	// Assertion error (wrong type): no 'F' standing for type marking mess the assertion up (double inferred as its type)
		//.body("coord.lat", is("47.8"))  	// Assertion error (wrong type): not a String to be expected, a float value rather
;
		//	WARNING: Values such as 'temp', 'visibility' and 'humidity' are all
		// 	volatile: it may change as quick as Open Weather Map API could inform us.
		//	Hence, those variables are not mapped on this sprecific test.
...
=======
	...
	public void deveAcessarCidadeSemQueryParam() {
		...
		given().log().all()
		.when()
		.get(getOpenWeatherMap() + getPartialQuery())
		.then().log().all()
		.assertThat()
		.statusCode(HttpStatus.SC_OK)
		;
